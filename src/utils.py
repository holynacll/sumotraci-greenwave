...
def generate_routefile(route_filepath: str, trips_filepath: str):
    road_filepath = "data/road.net.xml"
    route_filepath = f"data/{route_filepath}"
    cmd = f"python {os.environ['SUMO_HOME']}/tools/randomTrips.py -n {road_filepath} -r {route_filepath} -o {trips_filepath} --seed 42 --validate --fringe-factor 1000 -p {settings.TRIPS_REPETITION_RATE}"
    cmd_list = cmd.split(" ")
